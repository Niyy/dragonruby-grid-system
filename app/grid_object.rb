class Grid_Object
    attr_accessor :col, :row, :w_tile, :h_tile

    ##
    # Summary
    # @param init_args - w_tile, which is the tile width.
    #       h_tile, which is the tile height.
    def initialize(init_args)
        @w_tile = init_args[:w_tile] != nil ? init_args[:w_tile] : 32
        @h_tile = init_args[:h_tile] != nil ? init_args[:h_tile] : 32
    end


    def to_grid(coord)
        col_tile = coord[:x] / @w_tile 
        row_tile = coord[:y] / @h_tile

        return {
            col: col_tile.floor,
            row: row_tile.floor
        }
    end


    def to_coord(tile)
        x_tile = (tile[:col] * @w_tile)
        y_tile = (tile[:row] * @h_tile)

        return {
            x: x_tile,
            y: y_tile,
        }
    end


    # 1. Create a serialize method that returns a hash with all of
    #    the values you care about.
    def serialize()
        { w_tile: @w_tile, h_tile: @h_tile, col: @col, row: @row }
    end


    # 2. Override the inspect method and return ~serialize.to_s~.
    def inspect()
        serialize.to_s()
    end


    # 3. Override to_s and return ~serialize.to_s~.
    def to_s()
        serialize.to_s()
    end
end